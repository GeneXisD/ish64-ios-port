...
// NOTE: This header likely defines Linux guest errno codes (e.g. _ENOMEM, etc).
// It may also interfere with the host errno macro on Apple.
...
// ---- Host errno shim --------------------------------------------------------
// Avoid collisions with "kernel/errno.h" (guest errno system).
// Always returns the host (Darwin) errno value, even if `errno` got undefined.
#if defined(__APPLE__)
extern int *__error(void);
static inline int host_errno_value(void) { return *(__error()); }
#else
static inline int host_errno_value(void) { return errno; }
#endif
// -----------------------------------------------------------------------------
    ...
    for (page_t page = start; page < start + pages; mem_next_page(mem, &page)) {
        ...
        if (--data->refcount == 0) {
            if (data->data != vdso_data) {
                ...
                if (err != 0) {
                    die("munmap(%p, %lu) failed: %s",
                        data->data,
                        (unsigned long)data->size,
                        strerror(host_errno_value()));
                }
            }
            if (data->fd != NULL) {
                fd_close(data->fd);
