...
static int open_flags_fake_from_real(int flags) {
    int fake_flags = 0;
    if ((flags & O_ACCMODE) == O_RDONLY) fake_flags |= O_RDONLY_;
    if ((flags & O_ACCMODE) == O_WRONLY) fake_flags |= O_WRONLY_;
    if ((flags & O_ACCMODE) == O_RDWR)   fake_flags |= O_RDWR_;
...
#if defined(__linux__)
    char pathbuf[64];
    snprintf(pathbuf, sizeof(pathbuf), "/proc/self/fd/%d", dirfd);
...
/* copy host struct stat into project statbuf */
static void copy_stat(struct statbuf *fake_stat, const struct stat *real_stat) {
...
#if defined(st_atim) || defined(__APPLE__) || defined(__linux__) || defined(__unix__) || defined(__CYGWIN__)
...
int realfs_stat(struct mount *mount, const char *path, struct statbuf *fake_stat) {
    ...
    int tfd = openat(mount->root_fd, fixed, O_RDONLY);
    if (tfd >= 0) {
        if (fstat(tfd, &real_stat) < 0) {
            close(tfd);
            return errno_map();
        }
        close(tfd);
        ...
        snprintf(fullpath, sizeof(fullpath), "%s/%s", mount->source, fixed);
        if (lstat(fullpath, &real_stat) < 0)
            return errno_map();
    }
...
void realfs_opendir(struct fd *fd) {
    if (fd->dir == NULL) {
        ...
        fd->dir = ish_fdopendir_fallback(dirfd);
        assert(fd->dir != NULL);
    }
}
...
off_t realfs_lseek(struct fd *fd, off_t offset, int whence) {
    if (fd->dir != NULL && whence == LSEEK_SET) {
        realfs_seekdir(fd, (unsigned long) offset);
        return offset;
    }
...
int realfs_mmap(struct fd *fd, struct mem *mem, page_t start, pages_t pages,
